...
export default function UserAuth() {
  ...
  const handleBlur = (evt: React.FocusEvent<HTMLInputElement>) => {
    switch (evt.target.id) {
      case "email": {
        emailInputValidator(evt.target.value);
        return;
      }

      case "username": {
        usernameInputValidator(evt.target.value);
        return;
      }

      case "password": {
        passwordInputValidator(evt.target.value);
        return;
      }
    }
  };
  ...
  return (
    <section className="flex flex-col justify-center gap-8 flex-1 max-w-lg w-full mx-auto px-4 md:px-6">
      ...
      {mode !== "login" ? (
        <Input
          inputProps={{ type: "text", placeholder: "Username", id: "username" }}
          leadingIcon={
            <div className="shrink-0 select-none text-base text-gray-500 sm:text-sm/6">
              <svg
                xmlns="http://www.w3.org/2000/svg"
